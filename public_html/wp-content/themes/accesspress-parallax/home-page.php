...
if(!empty($sections)):
	foreach ( $sections as $section ) :
		...
		// если это страница, то начинаем с ней  работу
		if ( ! empty( $section['page'] ) ): ?>
			<section class="parallax-section clearfix<?php echo $googlemapclass . " " . $layout; ?>" id="<?php echo "section-" . $page->ID; ?>"> <?php
				if ( ! empty( $image ) && $overlay != "overlay0" ) : ?>
					<div class="overlay"></div> <?php
				endif;
				if ( $layout != "googlemap_template" ) : ?>
					<div class="mid-content"> <?php
				endif;
						// создаём экземпляр страницы что бы дальше ее выводить
						$query = new WP_Query( 'page_id=' . $section['page'] );

						// вывод подстраниц в цикле
						while ( $query->have_posts() ) : $query->the_post();
							if ( $layout != "action_template" && $layout != "blank_template" && $layout != "googlemap_template" ): ?>

								<!-- вывод заголовка подстраницы -->
								<h1><span><?php the_title(); ?></span></h1>
								<div class="parallax-content"> <?php

									// вывод контента подстраницы
									if ( get_the_content() != "" ) : ?>
